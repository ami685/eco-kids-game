...
const SuccessModal = props => {
  ...
  return (
    <SuccessBox id="success" onClick={handleClose}>
      <SC.ModalHeader>Bien Joué !</SC.ModalHeader>
      <SC.ModalHeader>
        <SC.OrangeText>{props.item.name} </SC.OrangeText> va dans la <SC.OrangeText> {props.item.bin}</SC.OrangeText> {['yard waste', 'special programs'].includes(props.item.bin.toLowerCase()) ? '' : ' bac'}.
      </SC.ModalHeader>
      <SC.BadgeBox>
      ...
      <Button primary handleClick={handleClose} label="D'accord" />
    </SuccessBox>
  );
};
